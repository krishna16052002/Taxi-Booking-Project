...
async function initializeSocket(server) {
  ...
  io.on('connection', (socket) => {
    ...
    const job = cron.schedule('* * * * * *', async () => {
      ...
      const ride = await createrideModel.find({ assigned: "assigning" });

      for (const data of ride) {
        if (nearestdriver == false) {
          if (data.created) {
            job.start();
    ...
    // let startTime; // Declare the startTime variable outside the function
    // const job = cron.schedule('* * * * * *', async () => {
    ...
    //     const currentTime = new Date();
    //     // console.log(currentTime.getTime());
    //     const elapsedTimeInSeconds = Math.floor((currentTime.getTime() - startTime.getTime()) / 1000);
    //     console.log(elapsedTimeInSeconds);        
    //     if (elapsedTimeInSeconds >=10) {
    //       console.log('Cron job ended.');
    //       const result1 = await driverModel.findByIdAndUpdate(driver_id, { assign: "0" }, { new: true });
    //       io.emit('driverUpdated', result1);


    //       const result = await createrideModel.findByIdAndUpdate(_id, { driver_id: null, assigned: "pending" }, { new: true });
    ...
    socket.on('assigndriver', async (data) => {
      ...
      try {
        ...
        await driver.save();
        // console.log(driver);
        const ride = await createrideModel.findByIdAndUpdate(_id, { driver_id: driver_id, assigned: "assigning", created: Date.now() }, { new: true })
        await ride.save()
        // console.log(ride);
    ...
    socket.on("assignnearestdriverdata", async (data) => {
      ...
      try {
        ...
        await driver.save();
        // // console.log(driver);
        const ride = await createrideModel.findByIdAndUpdate(data._id, { driver_id: firstdriver._id, assigned: "assigning", created: Date.now() }, { new: true })
        // await ride.save()
